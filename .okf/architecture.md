...
| `src/client/index.ts` | UI host: shadow-DOM picker, hotkey listener, pick mode with outline sync, shift+click multi-selection (numbered boxes, up to 5 elements total), popup with a To field (collapsed by default, showing the preselected target's name/status/branch/pane; `Down` or a click expands the agent list below it), a listbox where the two spawn rows (`+ agent here` / `+ agent in worktree`) are options alongside the agents so `Enter`/`Send` on one spawns then sends in a single step, the screenshot switch (shown only when live state reports `screenshot: 'available'`, persisted in `localStorage['herdr:shot']`), copy fallback, toast, in-flight outline driven by `herdr:status` HMR events (falls back to polling `/state`) with one status vocabulary shared by the list's status dots, the outline and the chips (amber working, green idle/done, red blocked); exposes `window.__herdr = { version, describe, outline, pick, close, inflight, selection, screenshotEnabled }` for e2e/bench |
